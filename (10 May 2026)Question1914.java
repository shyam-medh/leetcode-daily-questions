...
class Solution {
    public int[][] rotateGrid(int[][] grid, int k) {
        ...
        int tL = Math.min(rows, cols) / 2;
        for(int l = 0; l < tL; l++){
            ArrayList<Integer> elements = new ArrayList<>();

            int top = l, left = l;
            int bottom = rows - l - 1, right = cols - l - 1;

            for(int col = left; col <= right; col++) elements.add(grid[top][col]);
            for(int row = top + 1; row <= bottom - 1; row++) elements.add(grid[row][right]);
            for(int col = right; col >= left; col--) elements.add(grid[bottom][col]);
